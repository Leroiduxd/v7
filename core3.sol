...
contract BrokexCore {
    ...
    /**
     * @notice Exécute un ordre Limit ou Stop en attente.
     * @dev Fusion de la logique interne et externe pour optimisation.
     * @param tradeId L'ID de l'ordre à exécuter.
     * @param oracleProof Preuve de prix Supra Oracle.
     */
    function executeOrder(uint256 tradeId, bytes calldata oracleProof) external {
        ...
        // 2. Récupération et vérification du prix Oracle
        uint256 price1e6 = _getVerifiedPrice(oracleProof, t.assetId);
        
        // 3. Logique d'exécution Limit vs Stop
        bool executable;
        if (t.isLimit) {
            // Limit: Acheter plus bas ou Vendre plus haut que la cible
        ...
        // 4. Calcul du prix final avec Spread (Slippage inclus)
        ...
        // 8. Interaction avec le Vault et Event
