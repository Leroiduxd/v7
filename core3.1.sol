...
contract BrokexCore {
    ...
    struct FundingState {
        uint64 lastUpdate; uint128 longFundingIndex; uint128 shortFundingIndex;
    }
    ...
    // ----------------------------------------------------------------
    // CORE TRADING LOGIC (OPTIMIZED STACK)
    // ----------------------------------------------------------------

    // 1. OPEN MARKET (Stack Optimized)
    function openMarketPosition(address trader, uint32 assetId, bool isLong, uint8 leverage, int32 lotSize, uint48 stopLoss, uint48 takeProfit, bytes calldata oracleProof) external onlyPaymaster {
        ...
        // SCOPE 3: Finance Math
        {
            margin6 = calculateMargin6(assetId, entryPrice, uint32(lotSize), leverage);
            lpLocked6 = calculateLockedCapital(assetId, entryPrice, uint32(lotSize), leverage);
            tradeId = ++nextTradeID;
        }
    ...
    function placeOrder(address trader, uint32 assetId, bool isLong, bool isLimit, uint8 leverage, int32 lotSize, uint48 targetPrice, uint48 stopLoss, uint48 takeProfit) external onlyPaymaster {
        ...
        {
            margin6 = calculateMargin6(assetId, uint256(targetPrice), uint32(lotSize), leverage);
            lpLocked6 = calculateLockedCapital(assetId, uint256(targetPrice), uint32(lotSize), leverage);
            tradeId = ++nextTradeID;
        }

        trades[tradeId] = Trade({
            trader: trader, assetId: assetId, isLong: isLong, isLimit: isLimit, leverage: leverage, 
            openPrice: targetPrice, state: 0, openTimestamp: uint32(block.timestamp), fundingIndex: 0, 
            closePrice: 0, lotSize: lotSize, stopLoss: stopLoss, takeProfit: takeProfit, 
    ...
    function _calculateNetPnl(Trade storage t, uint256 price1e6, uint256 tradeId) internal view returns (int256) {
        ...
        int256 rawPnl = (delta * int256(uint256(uint32(t.lotSize))) * int256(uint256(a.numerator))) / int256(uint256(a.denominator));
    ...
    function _updateExposure(uint32 assetId, int32 lotSize, uint48 price, bool isLong, bool increase) internal {
        ...
        if (isLong) {
            if (increase) {
                ...
                e.longLots -= lotSize; e.longValueSum = _safeSub(e.longValueSum, value);
            }
        } else {
            if (increase) {
                ...
                e.shortLots -= lotSize; e.shortValueSum = _safeSub(e.shortValueSum, value);
            }
        }
    }
    ...
    function _updateExposureLimits(uint32 assetId, uint64 lpLocked, uint64 margin, bool isLong, bool increase) internal {
        Exposure storage e = exposures[assetId];
        uint128 locked = uint128(lpLocked); uint128 marg = uint128(margin);
        if (isLong) {
            if (increase) { e.longMaxProfit += locked; e.longMaxLoss += marg; }
            else { e.longMaxProfit = _safeSub(e.longMaxProfit, locked); e.longMaxLoss = _safeSub(e.longMaxLoss, marg); }
        } else {
            if (increase) { e.shortMaxProfit += locked; e.shortMaxLoss += marg; }
            else { e.shortMaxProfit = _safeSub(e.shortMaxProfit, locked); e.shortMaxLoss = _safeSub(e.shortMaxLoss, marg); }
        }
    }
    ...
    function _getVerifiedPrice(bytes calldata _bytesProof, uint32 _assetId) internal returns (uint256) {
        ...
        uint256 len = info.pairs.length; bool found; uint256 index;
        for(uint256 i = 0; i < len; i++) { if(info.pairs[i] == uint256(_assetId)) { index = i; found = true; break; } }
        if (!found) revert PairNotInProof();
        
        uint256 oracleTime = info.timestamp[index];
        if (oracleTime > 1000000000000) oracleTime = oracleTime / 1000;
        ...
        uint256 rawPrice = info.prices[index]; uint256 decimals = info.decimal[index];
        return (decimals > 6) ? rawPrice / (10 ** (decimals - 6)) : (decimals < 6) ? rawPrice * (10 ** (6 - decimals)) : rawPrice;
    }
    ...
    function calculateSpread(uint32 assetId, bool isLong, bool isOpening, uint32 lotSize) public view returns (uint256) {
        ...
        uint256 num = (L > S) ? uint256(L - S) : uint256(S - L); uint256 den = uint256(L + S + 2);
        uint256 p = ((num * 1e18) / den) ** 2 / 1e18;
        return ((L > S && isLong) || (S > L && !isLong)) ? (base * (1e18 + 3 * p)) / 1e18 : base;
    }
    ...
    function calculateWeekendFunding(uint256 tradeId) public view returns (uint256) {
        ...
        uint256 openWeek = (uint256(t.openTimestamp) + 259200) / 604800;
        uint256 currentWeek = (closeTs + 259200) / 604800;
        if (currentWeek <= openWeek) return 0;
        return (currentWeek - openWeek) * uint256(a.weekendFunding) * uint256(uint32(t.lotSize));
    }

    function calculateMargin6(uint32 assetId, uint256 entryPrice, uint32 lotSize, uint8 leverage) public view returns (uint256) {
        return _getNotionalValue(assetId, entryPrice, lotSize) / uint256(leverage);
    }
    ...
    function calculateLockedCapital(uint32 assetId, uint256 entryPrice, uint32 lotSize, uint8 leverage) public view returns (uint256) {
        ...
        uint256 maxProfitLev = (notional / uint256(leverage) * uint256(a.securityMultiplier)) / 100;
        uint256 physProfit = (entryPrice * uint256(a.maxPhysicalMove) / 100) * uint256(lotSize) * uint256(a.numerator) / uint256(a.denominator);
    ...
    function calculateLiquidationPrice(uint256 tradeId) public view returns (uint256) {
        Trade memory t = trades[tradeId]; Asset memory a = assets[t.assetId]; FundingState memory f = fundingStates[t.assetId];
        ...
        uint256 spreadCost = _getNotionalValue(t.assetId, calculateSpread(t.assetId, !t.isLong, false, uint32(t.lotSize)), uint32(t.lotSize));
        ...
        return t.isLong ? (deltaPrice >= openPrice ? 0 : openPrice - deltaPrice) : openPrice + deltaPrice;
    }
    ...
    function _updateFundingRate(uint32 assetId) internal {
        ...
        (uint256 lR, uint256 sR) = _computeFundingRateQuadratic(L, S, uint256(a.baseFundingRate));
        f.longFundingIndex += uint128(lR); f.shortFundingIndex += uint128(sR);
        f.lastUpdate = uint64(block.timestamp);
    }

    function _computeFundingRateQuadratic(uint256 L, uint256 S, uint256 base) internal pure returns (uint256, uint256) {
        if (L == S) return (base, base);
        uint256 num = (L > S) ? (L - S) : (S - L); uint256 den = L + S + 2;
        uint256 p = ((num * 1e18) / den) ** 2 / 1e18;
        uint256 dom = (base * (1e18 + 3 * p)) / 1e18;
        return (L > S) ? (dom, base) : (base, dom);
    }
    ...
    function updateUnrealizedPnl(bytes calldata singleProof, uint32[] calldata assetIds) external returns (uint64 runId, bool runCompleted, int256 currentPnl) {
        ...
        if (currentPnlRunId == 0 || block.timestamp > pnlRuns[currentPnlRunId].startTimestamp + 2 minutes || pnlRuns[currentPnlRunId].completed) {
            ...
            run.runId = currentPnlRunId; run.startTimestamp = uint64(block.timestamp);
        ...
        for (uint256 i = 0; i < assetIds.length; i++) {
            ...
            run.cumulativePnlX6 += _calculateAssetPnlCapped(assetId, price1e6);
    ...
    function _calculateAssetPnlCapped(uint32 assetId, uint256 currentPrice1e6) internal view returns (int256) {
        ...
        if (e.longLots > 0) {
            ...
            longPnl = int256(currentVal) - int256(uint256(e.longValueSum));
        ...
        if (e.shortLots > 0) {
            ...
            shortPnl = int256(uint256(e.shortValueSum)) - int256(currentVal);
    ...
    function _extractPriceFromInfo(ISupraOraclePull.PriceInfo memory info, uint32 _assetId) internal view returns (uint256) {
        uint256 len = info.pairs.length; bool found; uint256 index;
        for(uint256 i = 0; i < len; i++) { if(info.pairs[i] == uint256(_assetId)) { index = i; found = true; break; } }
        if (!found) revert PairNotInProof();
        uint256 rawPrice = info.prices[index]; uint256 decimals = info.decimal[index];
    ...
    function getExposureAndAveragePrices(uint32 assetId) public view returns (uint32 longLots, uint32 shortLots, uint256 avgLongPrice, uint256 avgShortPrice) {
        ...
        longLots = uint32(e.longLots); shortLots = uint32(e.shortLots);
    ...
    function getTradeStatesFromList(uint256[] calldata tradeIds) external view returns (uint8[] memory states) {
        uint256 len = tradeIds.length; if (len > 1000) revert("List too long");
        states = new uint8[](len);
        for (uint256 i = 0; i < len; i++) { states[i] = trades[tradeIds[i]].state; }
    }
    ...
    function getTradeSLTPFromList(uint256[] calldata tradeIds) external view returns (uint48[] memory stopLosses, uint48[] memory takeProfits) {
        uint256 len = tradeIds.length; if (len > 1000) revert("List too long");
        stopLosses = new uint48[](len); takeProfits = new uint48[](len);
        for (uint256 i = 0; i < len; i++) { Trade storage t = trades[tradeIds[i]]; stopLosses[i] = t.stopLoss; takeProfits[i] = t.takeProfit; }
    }

    function updateLotSize(uint32 assetId, uint32 newNum, uint32 newDen) external onlyOwner {
            if (!assets[assetId].listed) revert UnknownAsset();
            Exposure storage e = exposures[assetId];
            // On ne change la taille du lot que si l'exposition est vide (sécurité mathématique)
            if (e.longLots != 0 || e.shortLots != 0) revert ExposureNotZero();
