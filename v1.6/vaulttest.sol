...
contract BrokexVault {
    ...
    // Constants
    // --------------------------------------
    uint256 public constant epochDuration = 1 minutes;
    ...
    */
    function rollEpoch(int256 unrealizedPnl) external {
        uint256 epoch = currentEpoch;
    ...
    */
    function claimWithdraw(uint256 withdrawEpoch) external {
        ...
        // Mode test : on crédite directement le solde libre interne
        freeBalance[msg.sender] += amount;
    ...
    */
    ...
    function lockTraderFunds(address trader, uint256 amount) external  {
    ...
    function unlockTraderFunds(address trader, uint256 amount) external  {
        require(trader != address(0), "trader zero");
        require(amount > 0, "amount zero");
