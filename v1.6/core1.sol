...
library BrokexLibrary {
  ...
  struct Asset {
    ...
    function calculateNetPnl(
        ...
        int256 rawPnl18 =
            (delta * lotSize256 * int256(uint256(a.numerator)) * 1e12) /
            int256(uint256(a.denominator));

        uint256 currentIdx = t.isLong ? f.longFundingIndex : f.shortFundingIndex;
        uint256 deltaIndex = currentIdx - t.fundingIndex;

        uint256 exitNotional6 =
            (exitPrice * uint256(uint32(sizeToCalc)) * uint256(a.numerator)) /
            uint256(a.denominator);
...
contract BrokexCore {
    ...
    function openMarketPosition(
        ...
        if (leverage < 1 || leverage > assets[assetId].maxLeverage) revert BadLev();
    ...
    function _finalizeClose(
        ...
        (int256 netPnl18, uint256 exitPrice, uint256 extraFeesUsdc) = BrokexLibrary.calculateNetPnl(
            t,
            assets[t.assetId],
