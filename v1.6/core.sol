...
interface IBrokexVault {
   function lockTraderFunds(address trader, uint256 amount6) external;
    ...
    function collectCommissionFromLocked(address trader, uint256 commission6) external;
...
contract BrokexCore {
    ...
    mapping(uint256 => uint256) public reservedCommission;
    ...
    function _recomputeAndSyncLpLock(uint32 assetId, bool isClosing) internal {
        ...
        uint256 directional = uint256(e.longMaxProfit) > uint256(e.shortMaxProfit)
            ? uint256(e.longMaxProfit)
            : uint256(e.shortMaxProfit);

        uint256 overlap = 0;
        if (avgShort > avgLong && e.longLots > 0 && e.shortLots > 0) {
            uint256 matchedLots = uint256(uint32(e.longLots < e.shortLots ? e.longLots : e.shortLots));
            overlap = (avgShort - avgLong) * matchedLots;
        }

        uint256 theoreticalNewLock = directional + overlap;
        uint256 effectiveNewLock = theoreticalNewLock;
    ...
    function openMarketPosition(
        ...
        // Cap max de gain individuel du trade
        ...
        // Ces valeurs restent stockées au niveau trade
        // pour le cap individuel de gain/perte
        ...
        // 2. On réserve côté trader marge + commission
        brokexVault.lockTraderFunds(trader, margin6 + commission6);

        // 3. On recalcule et synchronise le lock LP agrégé par actif
        _recomputeAndSyncLpLock(assetId, false);

        // 4. Comme c'est un market order, la commission est prélevée immédiatement
    ...
    function placeOrder(
        ...
        // On réserve marge + commission dès la création de l'ordre
        brokexVault.lockTraderFunds(trader, margin6 + commission6);

        // On garde la commission réservée côté Core jusqu'à l'exécution ou l'annulation
        reservedCommission[tradeId] = commission6;
    ...
    function executeOrder(uint256 tradeId, bytes calldata oracleProof) external {
        ...
        // Spread d'ouverture à l'exécution
        uint256 spreadWad = BrokexLibrary.calculateSpread(
            assets[t.assetId],
            exposures[t.assetId],
            t.isLong,
        ...
        // L'ordre devient une vraie exposition ouverte
        ...
        // Sync du lock LP agrégé par actif
        _recomputeAndSyncLpLock(t.assetId, false);

        // Encaissement réel de la commission réservée
        uint256 commission6 = reservedCommission[tradeId];
        if (commission6 > 0) {
            brokexVault.collectCommissionFromLocked(t.trader, commission6);
            reservedCommission[tradeId] = 0;
        }
    ...
    function cancelOrder(address trader, uint256 tradeId) external onlyPaymaster {
        ...
        // On rend marge + commission réservée
        brokexVault.unlockTraderFunds(trader, uint256(t.marginUsdc) + commission6);
    ...
    function _finalizeClose(
        ...
        // Net PnL calculé sur la portion fermée
        (int256 netPnl18, uint256 exitPrice) = BrokexLibrary.calculateNetPnl(
            t,
            assets[t.assetId],
        ...
        // Conversion vers 1e6 pour le Vault minimaliste
        ...
        // 2. Sync du lock LP agrégé (mode closing = ne jamais relocker plus)
        _recomputeAndSyncLpLock(t.assetId, true);

        // 3. Mise à jour du closePrice moyen
        ...
        // 4. Mise à jour du trade restant
        ...
        // 5. On rend d'abord la marge concernée au trader
        brokexVault.unlockTraderFunds(t.trader, marginToRelease);

        // 6. Puis on règle le PnL sur les soldes libres
        if (actualPnl6 != 0) {
    ...
    function liquidatePosition(uint256 tradeId, bytes calldata oracleProof) external {
        ...
        // 2. Sync du lock LP agrégé en mode fermeture
        _recomputeAndSyncLpLock(t.assetId, true);

        // 3. Calcul du prix de sortie réel (avec spread) pour archive
        uint256 spreadWad = BrokexLibrary.calculateSpread(
            assets[t.assetId],
            exposures[t.assetId],
            !t.isLong,
            false,
            uint32(remainingLots)
        );
        uint256 spreadAmount = (price1e6 * spreadWad) / 1e18;
        uint256 exitPrice = t.isLong
            ? (price1e6 > spreadAmount ? price1e6 - spreadAmount : 0)
            : (price1e6 + spreadAmount);

        // 4. Marquage du trade comme totalement fermé
        t.state = 2;
        ...
        // 5. On rend d'abord la marge au freeBalance
        if (remainingMargin6 > 0) {
            brokexVault.unlockTraderFunds(t.trader, remainingMargin6);
        }

        // 6. Puis on applique une perte égale à toute la marge restante
        if (remainingMargin6 > 0) {
