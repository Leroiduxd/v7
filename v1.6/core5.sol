...
library BrokexLibrary {
    ...
    function computeAllowedDominanceBps(
        ...
        if (totalRisk <= uint256(a.imbalanceBufferUsd6)) {
            return type(uint256).max; // aucune contrainte sous buffer
        }
...
contract BrokexCore {
    ...
    constructor(address _oracle, address _libraryAddress) {
        ...
        if (_libraryAddress == address(0)) revert ZeroLibraryAddress();
    ...
    function listAsset(
        ...
        assets[assetId] = BrokexLibrary.Asset({
            ...
            // --- defaults ouverture / équilibre / concentration ---
    ...
    function setAssetImbalanceParams(
        ...
        if (newMaxRatioBps < 10000 || newMaxRatioBps > MAX_IMBALANCE_MAX_RATIO_BPS) {
    ...
    function executeOrder(
        ...
        // --- NEW: contrôles d'ouverture au moment où le risque devient réel ---
