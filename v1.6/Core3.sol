...
contract BrokexCore {
    ...
    function _recomputeAndSyncLpLock(uint32 assetId, bool isClosing) internal {
        ...
        // À la fermeture / liquidation, on ne relock jamais
        if (isClosing && newLock > oldLock) {
            newLock = oldLock;
        }

        if (newLock > oldLock) {
            uint256 deltaLock = newLock - oldLock;
            brokexVault.lockLpCapital(deltaLock);
        } else if (oldLock > newLock) {
            uint256 deltaUnlock = oldLock - newLock;
    ...
    function _finalizeClose(
        ...
        if (lotsToClose == remainingLots) {
            ...
            uint256 closeRatioWad = (uint256(uint32(lotsToClose)) * 1e18) /
                uint256(uint32(remainingLots));
            marginToRelease = (uint256(t.marginUsdc) * closeRatioWad) / 1e18;
            lpToRelease = (uint256(t.lpLockedCapital) * closeRatioWad) / 1e18;
        }
